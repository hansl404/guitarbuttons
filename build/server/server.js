...
if (!isProduction) {
  ...
  async function allAppFiles(project, app) {
  ...
  app.get("/:project/:app", async function(req, res, next) {
    let props = req.params;
    ...
    let files = await allAppFiles(props.project, props.app);
    if (files.length === 0) {
      let files = await staticFile(props.project, props.app);
      if (files.length > 0) {
        res.sendFile(path.resolve(files[0]));
      } else {
